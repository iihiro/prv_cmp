...
// CallbackFunctionEncResult

void CallbackFunctionEncResult::data_function(uint64_t code,
                                              const stdsc::Buffer& buffer,
                                              stdsc::StateContext& state)
{
    STDSC_LOG_INFO("Received encrypted results. (current state : %lu)",
                   state.current_state());
    STDSC_THROW_CALLBACK_IF_CHECK(
        kStateConnected <= state.current_state(),
        "Warn: must be generatedkeys state to receive encryped results.");
    ...
    //
    //std::shared_ptr<prvc_share::EncRecommendation> enc_recommendation_ptr(
    //  new prvc_share::EncRecommendation());
...
DEFINE_REQUEST_FUNC(CallbackFunctionEncResult);
DEFINE_DOWNLOAD_FUNC(CallbackFunctionEncResult);
...
//    sock.send_packet(
//      stdsc::make_data_packet(prvc_share::kControlCodeDataResult, size));
