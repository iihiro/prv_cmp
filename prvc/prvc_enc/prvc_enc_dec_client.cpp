...
struct DecClient<T>::Impl
{
    ...
    {
        stdsc::Buffer pubkey;
        client_.recv_data_blocking(prvc_share::kControlCodeDownloadPubkey,
                                   pubkey);
    ...
    void exec(T& args, std::shared_ptr<stdsc::ThreadException> te)
    {
        try
        {
            ...
            STDSC_LOG_INFO("Requesting connect to Decryptor.");
            client_.send_request_blocking(
              prvc_share::kControlCodeRequestConnect, retry_interval_usec,
              timeout_sec);

            download_pubkey(args.pubkey_filename.c_str());

            STDSC_LOG_INFO("Requesting disconnect to Decryptor.");
            client_.send_request_blocking(
              prvc_share::kControlCodeRequestDisconnect, retry_interval_usec,
              timeout_sec);
            client_.close();
        }
